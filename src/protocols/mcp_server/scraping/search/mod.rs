...
use super::types::{SearchResults, ImageSearchResults};
...
pub async fn perform_image_search(query: &str, num_results: usize, search_engine: &str) -> Result<ImageSearchResults> {
    eprintln!("🖼️ DEBUG: perform_image_search called with engine: {}", search_engine);
    match search_engine {
        "duckduckgo" => {
        ...
        _ => Err(anyhow::anyhow!("Unsupported image search engine: {}. Supported engines: google, bing, duckduckgo", search_engine)),
    }
}
