                        ...
                        match dc_result {
                            Ok(dc) => {
                                ...
                                runtime_clone.block_on(async {
                                    data_channels_clone.lock().await.insert(dc_id.clone(), dc);
                                });
                                ...
                                // Real send method
                                let dc_for_send = Arc::new(dc.clone());
                                let runtime_for_send = Arc::clone(&runtime);
                                let send_fn = unsafe { NativeFunction::from_closure(move |_, args, _context| {
                                    if !args.is_empty() {
                                        if let Ok(data) = args[0].to_string(_context) {
                                            let dc_clone = Arc::clone(&dc_for_send);
                                            let runtime_clone = Arc::clone(&runtime_for_send);
                                            let data_str = data.to_std_string_escaped();

                                            // Actually send data
                                            let _result = runtime_clone.block_on(async move {
                                                dc_clone.send_text(data_str).await
                                            });
                                        }
                                    }
                                    Ok(JsValue::undefined())
