...
impl GeolocationManager {
    ...
    pub fn setup_geolocation_api(
            ...
            .get(js_string!("navigator"), context)
        {
            nav.as_object()
                .map(|obj| obj.clone())
                .unwrap_or_else(|| JsObject::default(&context.intrinsics()))
        } else {
        ...
        let get_current_position_fn = unsafe {
            NativeFunction::from_closure(|_, args, context| {
                ...
                coords_obj.set(
                    js_string!("latitude"),
                    JsValue::from(latitude),
                    false,
                    context,
                )?;
                coords_obj.set(
                    js_string!("longitude"),
                    JsValue::from(longitude),
                    false,
                    context,
                )?;
                coords_obj.set(
                    js_string!("accuracy"),
                    JsValue::from(accuracy),
                    false,
                    context,
                )?;
        ...
        geolocation_obj.set(
            ...
            JsValue::from(get_current_position_fn.to_js_function(context.realm())),
            false,
            context,
        )?;
        ...
        geolocation_obj.set(
            ...
            JsValue::from(watch_position_fn.to_js_function(context.realm())),
            false,
            context,
        )?;
        ...
        geolocation_obj.set(
            ...
            JsValue::from(clear_watch_fn.to_js_function(context.realm())),
            false,
            context,
        )?;

        navigator.set(
            js_string!("geolocation"),
            JsValue::from(geolocation_obj),
            false,
            context,
        )?;
    ...
    fn ip_geolocation() -> Result<(f64, f64)> {
        ...
        if !response.status().is_success() {
            return Err(anyhow::anyhow!(
                "IP geolocation service returned error: {}",
                response.status()
            ));
        }
