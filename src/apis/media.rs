    ...
    fn setup_audio_context_api(&self, context: &mut Context) -> Result<(), boa_engine::JsError> {
        ...
        let audio_context_constructor = unsafe {
            NativeFunction::from_closure(move |_, _args, context| {
                ...
                let decode_audio_data_fn = NativeFunction::from_closure(|_, _args, ctx| {
                    let promise_obj = JsObject::default();

                    let then_fn =
                        NativeFunction::from_closure(move |_, callback_args, callback_ctx| {
                    ...
                    promise_obj.set(
                        js_string!("catch"),
                        JsValue::from(catch_fn.to_js_function(ctx.realm())),
                        false,
                        ctx,
                    )?;
                ...
                audio_ctx.set(
                    js_string!("decodeAudioData"),
                    JsValue::from(decode_audio_data_fn.to_js_function(context.realm())),
                    false,
                    context,
                )?;
    ...
    fn setup_speech_apis(&self, context: &mut Context) -> Result<(), boa_engine::JsError> {
        ...
        let speech_recognition_constructor = unsafe {
            NativeFunction::from_closure(|_, _args, context| {
                ...
                recognition.set(
                    js_string!("continuous"),
                    JsValue::from(false),
                    true,
                    context,
                )?;
                recognition.set(
                    js_string!("interimResults"),
                    JsValue::from(false),
                    true,
                    context,
                )?;
                ...
                let start_fn = NativeFunction::from_closure(|_, _, _| {
        ...
        context.register_global_property(
            ...
            JsValue::from(speech_recognition_fn.clone()),
            Attribute::all(),
        )?;
        context.register_global_property(
            js_string!("webkitSpeechRecognition"),
            JsValue::from(speech_recognition_fn),
            Attribute::all(),
        )?;
