...
impl EventManager {
    ...
    fn setup_event_constructor(&self, context: &mut Context) -> Result<(), boa_engine::JsError> {
        let event_constructor = unsafe { NativeFunction::from_closure(|_, args, context| {
            if args.is_empty() {
            ...
            let (bubbles, cancelable, composed) = if args.len() > 1 && args[1].is_object() {
                ...
                let bubbles = options.get(js_string!("bubbles"), context)
                    .unwrap_or(JsValue::Boolean(false))
                    .to_boolean();
                let cancelable = options.get(js_string!("cancelable"), context)
                    .unwrap_or(JsValue::Boolean(false))
                    .to_boolean();
                let composed = options.get(js_string!("composed"), context)
                    .unwrap_or(JsValue::Boolean(false))
                    .to_boolean();
            ...
            event_obj.set(js_string!("type"), JsValue::from(js_string!(event_type)), false, context)?;
            event_obj.set(js_string!("bubbles"), JsValue::from(bubbles), false, context)?;
            event_obj.set(js_string!("cancelable"), JsValue::from(cancelable), false, context)?;
            event_obj.set(js_string!("composed"), JsValue::from(composed), false, context)?;
            event_obj.set(js_string!("defaultPrevented"), JsValue::from(false), false, context)?;
            ...
            event_obj.set(js_string!("timeStamp"), JsValue::from(SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis() as f64), false, context)?;

            // preventDefault method
            let prevent_default_fn = unsafe { NativeFunction::from_closure(move |_, _args, _context| {
                Ok(JsValue::undefined())
            }) };
            event_obj.set(js_string!("preventDefault"), JsValue::from(prevent_default_fn.to_js_function(context.realm())), false, context)?;

            // stopPropagation method
            let stop_propagation_fn = unsafe { NativeFunction::from_closure(move |_, _args, _context| {
                Ok(JsValue::undefined())
            }) };
            event_obj.set(js_string!("stopPropagation"), JsValue::from(stop_propagation_fn.to_js_function(context.realm())), false, context)?;

            // stopImmediatePropagation method
            let stop_immediate_propagation_fn = unsafe { NativeFunction::from_closure(move |_, _args, _context| {
                Ok(JsValue::undefined())
            }) };
            event_obj.set(js_string!("stopImmediatePropagation"), JsValue::from(stop_immediate_propagation_fn.to_js_function(context.realm())), false, context)?;

            Ok(JsValue::from(event_obj))
        }) };

        context.register_global_property(js_string!("Event"), JsValue::from(event_constructor.to_js_function(context.realm())), Attribute::all())?;
        Ok(())
    }

    /// Setup CustomEvent constructor
    fn setup_custom_event_constructor(&self, context: &mut Context) -> Result<(), boa_engine::JsError> {
        let custom_events_clone = Arc::clone(&self.custom_events);
        let custom_event_constructor = unsafe { NativeFunction::from_closure(move |_, args, context| {
            if args.is_empty() {
                return Err(boa_engine::JsNativeError::typ()
            ...
            let event_type = args[0].to_string(context)?.to_std_string_escaped();
            let (bubbles, cancelable, composed, detail) = if args.len() > 1 && args[1].is_object() {
                let options = args[1].as_object().unwrap();
                let bubbles = options.get(js_string!("bubbles"), context)
                    .unwrap_or(JsValue::Boolean(false))
                    .to_boolean();
                ...
                let composed = options.get(js_string!("composed"), context)
                    .unwrap_or(JsValue::Boolean(false))
                    .to_boolean();
                let detail = options.get(js_string!("detail"), context)
        ...
        context.register_global_property(js_string!("CustomEvent"), JsValue::from(custom_event_constructor.to_js_function(context.realm())), Attribute::all())?;
        Ok(())
    }
    ...
    fn setup_event_target_methods(&self, context: &mut Context) -> Result<(), boa_engine::JsError> {
        ...
        let add_event_listener_fn = unsafe { NativeFunction::from_closure(move |_, args, _context| {
            if args.len() < 2 {
                return Err(boa_engine::JsNativeError::typ()
            ...
            let (capture, passive, once) = if args.len() > 2 {
                if args[2].is_boolean() {
                    ...
                    let capture = options.get(js_string!("capture"), _context)
                        .unwrap_or(JsValue::Boolean(false))
                        .to_boolean();
                    let passive = options.get(js_string!("passive"), _context)
                        .unwrap_or(JsValue::Boolean(false))
                        .to_boolean();
                    let once = options.get(js_string!("once"), _context)
                        .unwrap_or(JsValue::Boolean(false))
                        .to_boolean();
...

            {
                ...
                listeners.entry(event_type.clone()).or_insert_with(Vec::new).push(listener);
            }
        ...
        let remove_event_listener_fn = unsafe { NativeFunction::from_closure(move |_, args, _context| {
            if args.len() < 2 {
                return Err(boa_engine::JsNativeError::typ()
        ...
        let dispatch_event_fn = unsafe { NativeFunction::from_closure(move |_, args, context| {
            if args.is_empty() {
                return Err(boa_engine::JsNativeError::typ()
            ...
            if let Some(event) = event_obj.as_object() {
                let event_type = event.get(js_string!("type"), context)?
                    .to_string(context)?
                ...
                let dom_event = DomEvent {
                    ...
                    composed: event.get(js_string!("composed"), context).unwrap_or(JsValue::Boolean(false)).to_boolean(),
        ...
        window_obj.set(js_string!("addEventListener"), JsValue::from(add_event_listener_fn.to_js_function(context.realm())), false, context)?;
        window_obj.set(js_string!("removeEventListener"), JsValue::from(remove_event_listener_fn.to_js_function(context.realm())), false, context)?;
        window_obj.set(js_string!("dispatchEvent"), JsValue::from(dispatch_event_fn.to_js_function(context.realm())), false, context)?;
    ...
    fn setup_common_events(&self, context: &mut Context) -> Result<(), boa_engine::JsError> {
        ...
        let mouse_event_constructor = unsafe { NativeFunction::from_closure(|_, args, context| {
            if args.is_empty() {
                return Err(boa_engine::JsNativeError::typ()
            ...
            event_obj.set(js_string!("cancelable"), JsValue::from(true), false, context)?;
        ...
        context.register_global_property(js_string!("MouseEvent"), JsValue::from(mouse_event_constructor.to_js_function(context.realm())), Attribute::all())?;

        // Setup KeyboardEvent constructor
        let keyboard_event_constructor = unsafe { NativeFunction::from_closure(|_, args, context| {
            if args.is_empty() {
                return Err(boa_engine::JsNativeError::typ()
            ...
            event_obj.set(js_string!("bubbles"), JsValue::from(true), false, context)?;
            event_obj.set(js_string!("cancelable"), JsValue::from(true), false, context)?;
            event_obj.set(js_string!("key"), JsValue::from(js_string!("")), false, context)?;
            event_obj.set(js_string!("code"), JsValue::from(js_string!("")), false, context)?;
        ...
        context.register_global_property(js_string!("KeyboardEvent"), JsValue::from(keyboard_event_constructor.to_js_function(context.realm())), Attribute::all())?;

        // Setup FocusEvent constructor
        let focus_event_constructor = unsafe { NativeFunction::from_closure(|_, args, context| {
            if args.is_empty() {
                return Err(boa_engine::JsNativeError::typ()
        ...
        context.register_global_property(js_string!("FocusEvent"), JsValue::from(focus_event_constructor.to_js_function(context.realm())), Attribute::all())?;

        Ok(())
    }

    /// Add an event target (DOM element)
    pub fn add_event_target(&self, id: String, tag_name: String, parent_id: Option<String>) -> Result<()> {
    ...
    pub async fn dispatch_dom_event(&self, target_id: &str, event: DomEvent) -> Result<bool> {
        ...
        tracing::debug!("DOM event dispatched: {} on {}", event.event_type, target_id);
    ...
    pub async fn process_queued_events(&self) -> Result<usize> {
        ...
        for queued_event in events {
            ...
            tracing::debug!("Processing queued event: {} on {}", queued_event.event_type, queued_event.target_id);
        }
