...
impl MediaManager {
    ...
    fn setup_audio_context_api(&self, context: &mut Context) -> Result<(), boa_engine::JsError> {
        ...
        let audio_context_constructor = unsafe { NativeFunction::from_closure(move |_, _args, context| {
            ...
            let create_oscillator_fn = unsafe { NativeFunction::from_closure(move |_, _, ctx| {
                ...
                let osc_id_start = osc_id_clone.clone();
    ...
    fn setup_speech_apis(&self, context: &mut Context) -> Result<(), boa_engine::JsError> {
        ...
        context.register_global_property(js_string!("SpeechRecognition"), JsValue::from(speech_recognition_constructor.to_js_function(context.realm())), Attribute::all())?;
        context.register_global_property(js_string!("webkitSpeechRecognition"), JsValue::from(speech_recognition_constructor.to_js_function(context.realm())), Attribute::all())?;
