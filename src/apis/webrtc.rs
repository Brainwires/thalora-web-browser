...
impl WebRTCManager {
    ...
    pub fn setup_webrtc_api(&self, context: &mut Context) -> Result<(), boa_engine::JsError> {
        ...
        let rtc_peer_connection_constructor = unsafe {
            NativeFunction::from_closure(move |_, args, context| {
                    ...
                    match api_clone.new_peer_connection(config).await {
                        Ok(pc) => {
                            ...
                            peer_connections_clone
                                .lock()
                                .await
                                .insert(pc_id_clone, Arc::clone(&pc_arc));
                            Ok(pc_arc)
                        }
                        Err(e) => Err(e),
                    }
                ...
                match peer_connection_result {
                    Ok(pc_arc) => {
                        ...
                        let state =
                            match runtime_for_state.block_on(async { pc_arc.connection_state() }) {
                        ...
                        pc_obj.set(
                            ...
                            JsValue::from(js_string!("new")),
                            false,
                            context,
                        )?;
                        pc_obj.set(
                            js_string!("signalingState"),
                            JsValue::from(js_string!("stable")),
                            false,
                            context,
                        )?;
                        ...
                        let create_offer_fn =
                            NativeFunction::from_closure(move |_, _args, context| {
                                ...
                                let then_fn = NativeFunction::from_closure(
                                    move |_, callback_args, callback_context| {
                                        if !callback_args.is_empty()
                                            && callback_args[0].is_callable()
                                        {
                                            ...
                                            match offer_result {
                                                Ok(offer) => {
                                                    ...
                                                    offer_obj.set(
                                                        js_string!("sdp"),
                                                        JsValue::from(js_string!(offer.sdp)),
                                                        false,
                                                        callback_context,
                                                    )?;
                                ...
                                promise_obj.set(
                                    js_string!("catch"),
                                    JsValue::from(catch_fn.to_js_function(context.realm())),
                                    false,
                                    context,
                                )?;
                        ...
                        pc_obj.set(
                            ...
                            JsValue::from(create_offer_fn.to_js_function(context.realm())),
                            false,
                            context,
                        )?;
                                            ...
                                            && callback_args[0].is_callable()
                                        {
                                            ...
                                            match answer_result {
                                                Ok(answer) => {
                                                    ...
                                                    answer_obj.set(
                                                        js_string!("sdp"),
                                                        JsValue::from(js_string!(answer.sdp)),
                                                        false,
                                                        callback_context,
                                                    )?;
                        ...
                        pc_obj.set(
                            ...
                            JsValue::from(create_answer_fn.to_js_function(context.realm())),
                            false,
                            context,
                        )?;
                        ...
                        pc_obj.set(
                            ...
                            JsValue::from(set_local_description_fn.to_js_function(context.realm())),
                            false,
                            context,
                        )?;
                            ...
                            NativeFunction::from_closure(move |_, args, context| {
                                ...
                                match dc_result {
                                    Ok(_dc) => {
                                        ...
                                        let _dc_for_send =
                                            format!("dc_ref_{}", rand::random::<u32>());
                                        let _runtime_for_send = Arc::clone(&runtime_for_dc);
                                        let send_fn = NativeFunction::from_closure(
                                            move |_, args, _context| {
                                                if !args.is_empty() {
                                        ...
                                        dc_obj.set(
                                            js_string!("send"),
                                            JsValue::from(send_fn.to_js_function(context.realm())),
                                            false,
                                            context,
                                        )?;
                        ...
                        pc_obj.set(
                            ...
                            JsValue::from(create_data_channel_fn.to_js_function(context.realm())),
                            false,
                            context,
                        )?;
        ...
        context.register_global_property(
            ...
            JsValue::from(rtc_peer_connection_constructor.to_js_function(context.realm())),
            Attribute::all(),
        )?;

        // Real RTCIceCandidate constructor
        let rtc_ice_candidate_constructor = unsafe {
        ...
        context.register_global_property(
            js_string!("RTCIceCandidate"),
            JsValue::from(rtc_ice_candidate_constructor.to_js_function(context.realm())),
            Attribute::all(),
        )?;

        // Real RTCSessionDescription constructor
        let rtc_session_description_constructor = unsafe {
            NativeFunction::from_closure(|_, args, context| {
        ...
        context.register_global_property(
            js_string!("RTCSessionDescription"),
            JsValue::from(rtc_session_description_constructor.to_js_function(context.realm())),
            Attribute::all(),
        )?;
    ...
    fn setup_media_devices(&self, context: &mut Context) -> Result<(), boa_engine::JsError> {
        ...
        let navigator = if let Ok(nav) = context
            .global_object()
        ...
        let get_user_media_fn = unsafe {
            NativeFunction::from_closure(|_, _args, context| {
                let promise_obj = JsObject::default();

                let then_fn =
                    NativeFunction::from_closure(move |_, callback_args, callback_context| {
                        if !callback_args.is_empty() && callback_args[0].is_callable() {
                            ...
                            let get_tracks_fn =
                                NativeFunction::from_closure(|_, _, _callback_context| {
                            ...
                            stream_obj.set(
                                js_string!("getTracks"),
                                JsValue::from(
                                    get_tracks_fn.to_js_function(callback_context.realm()),
                                ),
                                false,
                                callback_context,
                            )?;
        ...
        media_devices.set(
            ...
            JsValue::from(get_user_media_fn.to_js_function(context.realm())),
            false,
            context,
        )?;
