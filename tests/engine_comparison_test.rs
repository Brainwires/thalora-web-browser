        ...
        for prop in boa_exclusive.iter().take(10) {
            if let Some(info) = boa_map.get(*prop) {
                println!(
                    "  - {}: {} ({})",
                    prop,
                    info.get("type").unwrap_or(&Value::Null),
                    if info
                        .get("isFunction")
...
async fn compare_builtin_constructors(
    ...
    for (boa_info, v8_info) in boa_list.iter().zip(v8_list.iter()) {
        let name = boa_info
            ...
            .and_then(|v| v.as_str())
            .unwrap_or("unknown");
        let boa_exists = boa_info
            .get("exists")
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
        let v8_exists = v8_info
            .get("exists")
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
        ...
        let boa_props = boa_info
            .get("prototypeProps")
            .and_then(|v| v.as_u64())
            .unwrap_or(0);
        let v8_props = v8_info
            .get("prototypeProps")
            .and_then(|v| v.as_u64())
            .unwrap_or(0);

        println!(
            "  {} {:<20} BOA: {} props, V8: {} props",
            status, name, boa_props, v8_props
        );
    }
...
async fn compare_language_features(
    ...
    let feature_tests = vec![
        ...
        (
            ...
            "(function(x = 5) { return x; })() === 5",
        ),
        (
            "ES6 Rest Parameters",
            "(function(...args) { return args.length; })(1, 2, 3) === 3",
        ),
        ("ES6 Spread Operator", "[...[1, 2, 3]].length === 3"),
        (
            "ES6 for...of",
            "{ let sum = 0; for (const x of [1, 2, 3]) sum += x; sum === 6; }",
        ),
        (
            "ES6 Map",
            "new Map().set('key', 'value').get('key') === 'value'",
        ),
        ...
        (
            "ES8 async/await",
            "(async function() { return await Promise.resolve(42); })() instanceof Promise",
        ),
...
async fn compare_type_behavior(
    ...
    for (test_name, test_code) in type_tests {
        ...
        let status = if boa_result == v8_result {
            "✅"
        } else {
            "⚠️"
        };
        ...
        println!(
            "  {} {:<25} BOA: {}, V8: {}",
            status, test_name, boa_result, v8_result
        );
    }
...
async fn compare_error_handling(
    ...
    for (error_type, test_code) in error_tests {
        ...
        let status = if boa_error == v8_error {
            "✅"
        } else {
            "⚠️"
        };
        ...
        println!(
            "  {} {:<25} BOA: {}, V8: {}",
            status, error_type, boa_error, v8_error
        );
    }
...
async fn compare_performance(
    ...
    let performance_tests = vec![
        (
            "Simple arithmetic",
            "let sum = 0; for (let i = 0; i < 10000; i++) sum += i; sum",
        ),
        (
            "Array operations",
            "const arr = Array(1000).fill(0).map((_, i) => i); arr.reduce((a, b) => a + b, 0)",
        ),
        (
            "Object creation",
            "const objs = []; for (let i = 0; i < 1000; i++) objs.push({id: i, value: i * 2}); objs.length",
        ),
        (
            "String manipulation",
            "let str = ''; for (let i = 0; i < 1000; i++) str += i.toString(); str.length",
        ),
        (
            "Function calls",
            "function test(x) { return x * 2; } let result = 0; for (let i = 0; i < 1000; i++) result += test(i); result",
        ),
    ];
    ...
    for (test_name, test_code) in performance_tests {
        ...
        let faster = if boa_time < v8_time {
            "🔵 BOA"
        } else {
            "🟡 V8"
        };
        ...
        println!(
            "  {:<25} BOA: {:>8.2}ms, V8: {:>8.2}ms {} ({:.1}x faster)",
            test_name,
...
async fn measure_execution_time(
    engine: &mut Box<dyn ThaloraBrowserEngine>,
    code: &str,
) -> std::time::Duration {
...
async fn test_engine_consistency_basic() -> Result<()> {
    ...
    for test in test_cases {
        ...
        println!(
            "  {} {:<35} BOA: {}, V8: {}",
            status, test, boa_result, v8_result
        );
    }
