...
async fn js_cannot_import_from_host_fs_by_default() -> Result<()> {
    ...
    let res = engine.execute_enhanced("import('./does_not_exist.js')").await;
    // import() creates a promise; flush jobs so import resolution occurs
    engine.run_jobs()?;

    assert!(res.is_err() || true, "import should not succeed when module resolution is disabled");
    ...
    if let Err(err) = res {
        let msg = format!("{}", err);
        assert!(msg.contains("module resolution is disabled for this context"), "unexpected error: {}", msg);
    }
