...
async fn test_native_websocket_implementation() {
    ...
    let result = browser
        .lock()
        .unwrap()
        .execute_javascript("WebSocket.OPEN")
        .await;
    match result {
        Ok(value) => {
            let value_str = format!("{:?}", value);
    ...
    let result = browser
        .lock()
        .unwrap()
        .execute_javascript("WebSocket.CLOSING")
        .await;
    match result {
        Ok(value) => {
            let value_str = format!("{:?}", value);
    ...
    let result = browser
        .lock()
        .unwrap()
        .execute_javascript("WebSocket.CLOSED")
        .await;
    match result {
        Ok(value) => {
            let value_str = format!("{:?}", value);
    ...
    match result {
        Ok(value) => {
            ...
            if value_str.contains("error") {
                println!(
                    "⚠️ WebSocket `new` not supported in this environment: {}",
                    value_str
                );
                ...
                println!(
                    "ℹ️ Unexpected result from WebSocket instance creation, continuing: {}",
                    value_str
                );
            }
        }
        Err(e) => {
            println!(
                "⚠️ Failed to create WebSocket instance, continuing: {:?}",
                e
            );
        }
    }
    ...
    if ws_instantiated {
        match result {
            Ok(value) => {
                ...
                assert!(
                    ...
                    "WebSocket properties access should not error: {}",
                    value_str
                );
                assert!(
                    value_str.contains("url"),
                    "WebSocket should have url property"
                );
                assert!(
                    value_str.contains("readyState"),
                    "WebSocket should have readyState property"
                );
            }
            Err(e) => {
        ...
        println!(
            "⚠️ Skipping WebSocket properties check because instantiation is not supported in this environment"
        );
    }
    ...
    if ws_instantiated {
        match result {
            Ok(value) => {
                ...
                assert!(
                    !value_str.contains("error"),
                    "WebSocket methods access should not error: {}",
                    value_str
                );
                assert!(
                    value_str.contains("function"),
                    "WebSocket methods should be functions"
                );
            }
            Err(e) => {
        ...
        println!(
            "⚠️ Skipping WebSocket methods check because instantiation is not supported in this environment"
        );
    }
