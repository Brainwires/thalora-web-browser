...
fn test_load_module_from_wat() {
    ...
    let result = tools.load_module(json!({
        "wat_text": ADD_WAT,
        "module_id": "test-add",
        "name": "Add Module"
    })).unwrap();
...
fn test_unload_module() {
    ...
    let result = tools.unload_module(json!({
        "module_id": "to-unload"
    })).unwrap();
    ...
    assert!(tools.unload_module(json!({ "module_id": "to-unload" })).is_err());
}
...
fn test_validate_valid_wat() {
    ...
    let result = tools.validate(json!({
        "wat_text": ADD_WAT
    })).unwrap();
...
fn test_disassemble_full_module() {
    ...
    let result = tools.disassemble(json!({
        "module_id": "disasm-test"
    })).unwrap();
...
async fn test_call_add_function() {
    ...
    let result = tools.call_function(json!({
        "module_id": "call-test",
        "function_name": "add",
        "args": [
            {"type": "i32", "value": 3},
...
async fn test_call_function_wrong_args() {
    ...
    let err = tools.call_function(json!({
        "module_id": "wrong-args",
        "function_name": "add",
...
async fn test_memory_write_and_read_hex() {
    ...
    // Read back as hex
    let read_result = tools.read_memory(json!({
        "module_id": "mem-test",
        "offset": 0,
        "length": 16,
        "format": "hex"
...
async fn test_memory_write_and_read_typed() {
    ...
    // Read back as i32
    let result = tools.read_memory(json!({
        "module_id": "mem-typed",
        "offset": 0,
        "format": "i32",
        "count": 1
...
async fn test_profile_function() {
    ...
    let result = tools.profile_function(json!({
        "module_id": "profile-test",
        "function_name": "add",
        "args": [
            {"type": "i32", "value": 10},
...
fn test_missing_source() {
    ...
    assert!(err.is_err());
    assert!(err.unwrap_err().to_string().contains("wasm_base64 or wat_text"));
}
