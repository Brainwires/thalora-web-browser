...
fn test_ai_memory_search() {
    ...
    // Search by query
    let search_response = harness.call_tool("ai_memory_search_research", json!({
        "query": "rust",
        "limit": 10
    ...
    // Search by tags
    let tag_search_response = harness.call_tool("ai_memory_search_research", json!({
        "tags": ["web"],
        "limit": 10
    })).expect("Tag search should succeed");

    assert_tool_success(&tag_search_response, Duration::from_secs(5)).expect("Tag search should be fast");
}
...
fn test_browser_click_element() {
    ...
    let response = harness.call_tool("browser_click_element", json!({
...
fn test_cdp_runtime_evaluate() {
    ...
    assert!(response_text.contains("2") || response_text.contains("result"),
...
fn test_cdp_dom_get_document() {
    ...
    let response = harness.call_tool("cdp_dom_get_document", json!({
        "depth": 2
    ...
    let response_text = response.content[0].get("text").unwrap().as_str().unwrap();
    assert!(response_text.contains("document") || response_text.contains("DOM") || response_text.contains("html"),
        ...
        match response {
            Ok(response) => {
                assert!(response.is_error,
                       "Tool {} should return error for missing required params", tool_name);
            }
            Err(_) => {
                // Failing during call is also acceptable
            }
...
fn test_tools_response_times() {
    ...
    let quick_tools = vec![
        ("ai_memory_search_research", json!({"query": "test", "limit": 1})),
...
fn test_tools_with_large_data() {
    ...
    match response {
        Ok(response) => {
            ...
            assert!(response.duration < Duration::from_secs(30),
...
fn test_tools_with_unicode_data() {
    ...
    assert!(!response.is_error, "Should handle Unicode data without error");
