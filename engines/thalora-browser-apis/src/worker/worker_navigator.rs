...
impl WorkerNavigator {
    ...
    pub fn create(context: &mut Context) -> JsResult<JsObject> {
        ...
        let hardware_concurrency_getter =
            BuiltInBuilder::callable(context.realm(), get_hardware_concurrency)
...
fn get_user_agent(this: &JsValue, _: &[JsValue], _: &mut Context) -> JsResult<JsValue> {
    let this_obj = this.as_object().ok_or_else(|| {
        boa_engine::JsNativeError::typ()
            .with_message("WorkerNavigator.userAgent getter called on non-object")
    })?;

    let data = this_obj
        .downcast_ref::<WorkerNavigatorData>()
        .ok_or_else(|| {
            boa_engine::JsNativeError::typ().with_message(
                "WorkerNavigator.userAgent getter called on non-WorkerNavigator object",
            )
        })?;
...
fn get_platform(this: &JsValue, _: &[JsValue], _: &mut Context) -> JsResult<JsValue> {
    let this_obj = this.as_object().ok_or_else(|| {
        boa_engine::JsNativeError::typ()
            .with_message("WorkerNavigator.platform getter called on non-object")
    })?;
    ...
    let data = this_obj
        ...
        .ok_or_else(|| {
            boa_engine::JsNativeError::typ().with_message(
                "WorkerNavigator.platform getter called on non-WorkerNavigator object",
            )
        })?;
...
fn get_language(this: &JsValue, _: &[JsValue], _: &mut Context) -> JsResult<JsValue> {
    let this_obj = this.as_object().ok_or_else(|| {
        boa_engine::JsNativeError::typ()
            .with_message("WorkerNavigator.language getter called on non-object")
    })?;
    ...
    let data = this_obj
        ...
        .ok_or_else(|| {
            boa_engine::JsNativeError::typ().with_message(
                "WorkerNavigator.language getter called on non-WorkerNavigator object",
            )
        })?;
...
fn get_languages(this: &JsValue, _: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    let this_obj = this.as_object().ok_or_else(|| {
        boa_engine::JsNativeError::typ()
            .with_message("WorkerNavigator.languages getter called on non-object")
    })?;
    ...
    let js_array =
        boa_engine::builtins::array::Array::array_create(languages.len() as u64, None, context)?;
...
fn get_online(this: &JsValue, _: &[JsValue], _: &mut Context) -> JsResult<JsValue> {
    let this_obj = this.as_object().ok_or_else(|| {
        boa_engine::JsNativeError::typ()
            .with_message("WorkerNavigator.onLine getter called on non-object")
    })?;
    ...
    let data = this_obj
        ...
        .ok_or_else(|| {
            boa_engine::JsNativeError::typ()
                .with_message("WorkerNavigator.onLine getter called on non-WorkerNavigator object")
        })?;
...
fn get_hardware_concurrency(this: &JsValue, _: &[JsValue], _: &mut Context) -> JsResult<JsValue> {
    let this_obj = this.as_object().ok_or_else(|| {
        boa_engine::JsNativeError::typ()
            .with_message("WorkerNavigator.hardwareConcurrency getter called on non-object")
    })?;
    ...
    let data = this_obj
        ...
        .ok_or_else(|| {
            boa_engine::JsNativeError::typ().with_message(
                "WorkerNavigator.hardwareConcurrency getter called on non-WorkerNavigator object",
            )
        })?;
