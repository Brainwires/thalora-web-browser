...
pub fn setup_parse_html_unsafe(realm: &boa_engine::realm::Realm) {
    let _document_constructor = realm.intrinsics().constructors().document().constructor();

    let _parse_html_unsafe_func = BuiltInBuilder::callable(realm, parse_html_unsafe)
