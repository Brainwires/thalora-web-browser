...
fn node_lookup_methods() {
    run_test_actions([
        ...
        TestAction::assert_eq(
            "node.lookupPrefix('http://www.w3.org/1999/xhtml')",
            JsValue::null(),
        ),
        ...
        TestAction::assert_eq(
            "node.isDefaultNamespace('http://www.w3.org/1999/xhtml')",
            false,
        ),
    ]);
}
...
fn node_is_default_namespace() {
    run_test_actions([
        ...
        TestAction::assert_eq(
            "node.isDefaultNamespace('http://www.w3.org/2000/svg')",
            false,
        ),
        TestAction::assert_eq(
            "node.isDefaultNamespace('http://example.com/custom')",
            false,
        ),
    ]);
}
...
fn node_lookup_namespace_uri() {
    run_test_actions([
        ...
        TestAction::assert_eq(
            ...
            js_str!("http://www.w3.org/XML/1998/namespace"),
        ),
        TestAction::assert_eq(
            "node.lookupNamespaceURI('xmlns')",
            js_str!("http://www.w3.org/2000/xmlns/"),
        ),
