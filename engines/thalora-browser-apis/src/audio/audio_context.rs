...
fn get_state(_this: &JsValue, _args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    let this = _this.as_object().ok_or_else(|| {
        JsNativeError::typ().with_message("'this' is not an AudioContext")
    })?;

    let data = this.downcast_ref::<AudioContextData>().ok_or_else(|| {
        JsNativeError::typ().with_message("'this' is not an AudioContext")
    })?;
...
fn get_sample_rate(_this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
    let this = _this.as_object().ok_or_else(|| {
        JsNativeError::typ().with_message("'this' is not an AudioContext")
    })?;
...
fn get_current_time(_this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
    let this = _this.as_object().ok_or_else(|| {
        JsNativeError::typ().with_message("'this' is not an AudioContext")
    })?;
...
fn get_base_latency(_this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
    let this = _this.as_object().ok_or_else(|| {
        JsNativeError::typ().with_message("'this' is not an AudioContext")
    })?;
...
fn get_output_latency(_this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
    let this = _this.as_object().ok_or_else(|| {
        JsNativeError::typ().with_message("'this' is not an AudioContext")
    })?;
...
fn get_destination(_this: &JsValue, _args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    obj.set(js_string!("numberOfInputs"), JsValue::from(1), false, context)?;
    obj.set(js_string!("numberOfOutputs"), JsValue::from(0), false, context)?;
    obj.set(js_string!("channelCount"), JsValue::from(2), false, context)?;
    obj.set(js_string!("maxChannelCount"), JsValue::from(2), false, context)?;
    Ok(obj.into())
}
...
fn create_gain(_this: &JsValue, _args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    let obj = JsObject::with_null_proto();
    obj.set(js_string!("numberOfInputs"), JsValue::from(1), false, context)?;
    obj.set(js_string!("numberOfOutputs"), JsValue::from(1), false, context)?;
    obj.set(js_string!("channelCount"), JsValue::from(2), false, context)?;
...
fn create_oscillator(_this: &JsValue, _args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    // Create an OscillatorNode-like object
    let obj = JsObject::with_null_proto();
    obj.set(js_string!("numberOfInputs"), JsValue::from(0), false, context)?;
    obj.set(js_string!("numberOfOutputs"), JsValue::from(1), false, context)?;
    obj.set(js_string!("channelCount"), JsValue::from(2), false, context)?;
...
fn create_buffer_source(_this: &JsValue, _args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    obj.set(js_string!("playbackRate"), playback_rate_param, false, context)?;
...
fn create_analyser(_this: &JsValue, _args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    obj.set(js_string!("frequencyBinCount"), JsValue::from(1024), false, context)?;
    obj.set(js_string!("minDecibels"), JsValue::from(-100.0), false, context)?;
    obj.set(js_string!("maxDecibels"), JsValue::from(-30.0), false, context)?;
    obj.set(js_string!("smoothingTimeConstant"), JsValue::from(0.8), false, context)?;
    ...
    let get_byte_frequency_data_fn = BuiltInBuilder::callable(context.realm(), analyser_get_byte_frequency_data)
        .name(js_string!("getByteFrequencyData"))
        .build();
    obj.set(js_string!("getByteFrequencyData"), get_byte_frequency_data_fn, false, context)?;

    let get_float_frequency_data_fn = BuiltInBuilder::callable(context.realm(), analyser_get_float_frequency_data)
        .name(js_string!("getFloatFrequencyData"))
        .build();
    obj.set(js_string!("getFloatFrequencyData"), get_float_frequency_data_fn, false, context)?;

    let get_byte_time_domain_data_fn = BuiltInBuilder::callable(context.realm(), analyser_get_byte_time_domain_data)
        .name(js_string!("getByteTimeDomainData"))
        .build();
    obj.set(js_string!("getByteTimeDomainData"), get_byte_time_domain_data_fn, false, context)?;

    let get_float_time_domain_data_fn = BuiltInBuilder::callable(context.realm(), analyser_get_float_time_domain_data)
        .name(js_string!("getFloatTimeDomainData"))
        .build();
    obj.set(js_string!("getFloatTimeDomainData"), get_float_time_domain_data_fn, false, context)?;
...
fn create_buffer(_this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    let obj = JsObject::with_null_proto();
    obj.set(js_string!("numberOfChannels"), JsValue::from(num_channels), false, context)?;
    obj.set(js_string!("length"), JsValue::from(length), false, context)?;
    obj.set(js_string!("sampleRate"), JsValue::from(sample_rate as f64), false, context)?;
    obj.set(js_string!("duration"), JsValue::from(length as f64 / sample_rate as f64), false, context)?;
    ...
    let get_channel_data_fn = BuiltInBuilder::callable(context.realm(), buffer_get_channel_data)
        ...
        .build();
    obj.set(js_string!("getChannelData"), get_channel_data_fn, false, context)?;
...
fn decode_audio_data(_this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    let buffer = create_buffer(_this, &[
...
fn create_audio_param(context: &mut Context, default: f64, min: f64, max: f64) -> JsResult<JsValue> {
    let obj = JsObject::with_null_proto();
    obj.set(js_string!("value"), JsValue::from(default), false, context)?;
    obj.set(js_string!("defaultValue"), JsValue::from(default), false, context)?;
    obj.set(js_string!("minValue"), JsValue::from(min), false, context)?;
    obj.set(js_string!("maxValue"), JsValue::from(max), false, context)?;
    obj.set(js_string!("automationRate"), JsString::from("a-rate"), false, context)?;
    ...
    let set_value_at_time_fn = BuiltInBuilder::callable(context.realm(), param_set_value_at_time)
        ...
        .build();
    obj.set(js_string!("setValueAtTime"), set_value_at_time_fn, false, context)?;

    let linear_ramp_fn = BuiltInBuilder::callable(context.realm(), param_linear_ramp_to_value_at_time)
        .name(js_string!("linearRampToValueAtTime"))
        .build();
    obj.set(js_string!("linearRampToValueAtTime"), linear_ramp_fn, false, context)?;

    let exponential_ramp_fn = BuiltInBuilder::callable(context.realm(), param_exponential_ramp_to_value_at_time)
        .name(js_string!("exponentialRampToValueAtTime"))
        .build();
    obj.set(js_string!("exponentialRampToValueAtTime"), exponential_ramp_fn, false, context)?;

    let set_target_at_time_fn = BuiltInBuilder::callable(context.realm(), param_set_target_at_time)
        .name(js_string!("setTargetAtTime"))
        .build();
    obj.set(js_string!("setTargetAtTime"), set_target_at_time_fn, false, context)?;

    let set_value_curve_fn = BuiltInBuilder::callable(context.realm(), param_set_value_curve_at_time)
        .name(js_string!("setValueCurveAtTime"))
        .build();
    obj.set(js_string!("setValueCurveAtTime"), set_value_curve_fn, false, context)?;

    let cancel_scheduled_fn = BuiltInBuilder::callable(context.realm(), param_cancel_scheduled_values)
        .name(js_string!("cancelScheduledValues"))
        .build();
    obj.set(js_string!("cancelScheduledValues"), cancel_scheduled_fn, false, context)?;

    let cancel_and_hold_fn = BuiltInBuilder::callable(context.realm(), param_cancel_and_hold_at_time)
        .name(js_string!("cancelAndHoldAtTime"))
        .build();
    obj.set(js_string!("cancelAndHoldAtTime"), cancel_and_hold_fn, false, context)?;
...
fn node_disconnect(_this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
    // In a real implementation, this would disconnect the node
    Ok(JsValue::undefined())
}

fn oscillator_start(_this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
    // In a real implementation, this would start the oscillator
    Ok(JsValue::undefined())
}

fn oscillator_stop(_this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
    // In a real implementation, this would stop the oscillator
    Ok(JsValue::undefined())
}

fn buffer_source_start(_this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
    // In a real implementation, this would start playback
    Ok(JsValue::undefined())
}

fn buffer_source_stop(_this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
...
fn analyser_get_byte_frequency_data(_this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
    // In a real implementation, this would fill the array with frequency data
    Ok(JsValue::undefined())
}

fn analyser_get_float_frequency_data(_this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
    // In a real implementation, this would fill the array with frequency data
    Ok(JsValue::undefined())
}

fn analyser_get_byte_time_domain_data(_this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
    // In a real implementation, this would fill the array with time domain data
    Ok(JsValue::undefined())
}

fn analyser_get_float_time_domain_data(_this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
    // In a real implementation, this would fill the array with time domain data
    Ok(JsValue::undefined())
}
...
fn buffer_get_channel_data(_this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    let this_obj = _this.as_object().ok_or_else(|| {
        JsNativeError::typ().with_message("'this' is not an AudioBuffer")
    })?;

    let length = this_obj.get(js_string!("length"), context)?
    ...
    let float32_constructor = context.intrinsics().constructors().typed_float32_array().constructor();
...
fn buffer_copy_from_channel(_this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
    // In a real implementation, this would copy data from the channel
    Ok(JsValue::undefined())
}

fn buffer_copy_to_channel(_this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
...
fn param_set_value_at_time(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
...
fn param_linear_ramp_to_value_at_time(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    let value = args.get_or_undefined(0).to_number(context)?;
...
fn param_exponential_ramp_to_value_at_time(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    let value = args.get_or_undefined(0).to_number(context)?;
    // For simplicity, just set the value immediately
    if let Some(this_obj) = this.as_object() {
        this_obj.set(js_string!("value"), JsValue::from(value), false, context)?;
...
fn param_set_target_at_time(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
...
fn param_set_value_curve_at_time(this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
    // For simplicity, this is a no-op
    Ok(this.clone())
}

fn param_cancel_scheduled_values(this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
    Ok(this.clone())
}

fn param_cancel_and_hold_at_time(this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
    Ok(this.clone())
}
