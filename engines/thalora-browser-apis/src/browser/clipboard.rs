...
fn clipboard_read_text(_this: &JsValue, _args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    let text = {
...
fn clipboard_write_text(_this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
...
fn create_clipboard_item(items: &HashMap<String, Vec<u8>>, context: &mut Context) -> JsResult<JsObject> {
    ...
    for (mime_type, data) in items {
        ...
        data_obj.set(js_string!(mime_type.clone()), js_string!(data_str), false, context)?;
    }

    // Create ClipboardItem object with getType method
    let clipboard_item = ObjectInitializer::new(context)
        .property(js_string!("types"), types_array, Attribute::READONLY | Attribute::ENUMERABLE)
...
fn clipboard_item_get_type(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
        ...
        if let Ok(value) = data_obj.get(js_string!(mime_type.clone()), context) {
            if !value.is_undefined() {
                ...
                let blob = ObjectInitializer::new(context)
                    ...
                    .property(js_string!("type"), js_string!(mime_type), Attribute::READONLY)
                    .property(js_string!("_bytes"), js_string!(value_str.clone()), Attribute::all())
                    .function(
...
fn blob_text(this: &JsValue, _args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    let this_obj = this.as_object().ok_or_else(|| {
        JsNativeError::typ().with_message("Blob.text called on non-object")
    })?;
