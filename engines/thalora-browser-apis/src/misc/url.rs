...
impl BuiltInConstructor for Url {
    ...
    fn constructor(
        ...
        let prototype =
            get_prototype_from_constructor(new_target, StandardConstructors::url, context)?;
...
fn set_href(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    let new_url = url::Url::parse(&new_href.to_std_string_escaped())
        .map_err(|e| JsNativeError::typ().with_message(format!("Invalid URL: {}", e)))?;
...
fn set_hostname(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    let _ = url_data
        .inner
        .set_host(Some(&new_hostname.to_std_string_escaped()));
    Ok(JsValue::undefined())
}
...
fn set_pathname(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    url_data
        .inner
        .set_path(&new_pathname.to_std_string_escaped());
    Ok(JsValue::undefined())
}
...
fn get_search_params(
    ...
    // Extract data before any context calls to avoid RefCell double-borrow
    let params = {
        let url_data = this_obj.downcast_ref::<UrlData>().ok_or_else(|| {
    ...
    let params_obj =
        JsObject::from_proto_and_data_with_shared_shape(context.root_shape(), prototype, params);
...
fn append(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    // Extract args before borrowing to avoid RefCell double-borrow if toString() re-enters
    let name = args.get_or_undefined(0).to_string(context)?;
    let value = args.get_or_undefined(1).to_string(context)?;

    let mut params = this_obj
        .downcast_mut::<UrlSearchParamsData>()
        .ok_or_else(|| {
            JsNativeError::typ().with_message(
                "URLSearchParams.prototype.append called on non-URLSearchParams object",
            )
        })?;
...
fn delete_param(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    let mut params = this_obj
        ...
        .ok_or_else(|| {
            JsNativeError::typ().with_message(
                "URLSearchParams.prototype.delete called on non-URLSearchParams object",
            )
        })?;
...
fn get_param(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    let params = this_obj
        ...
        .ok_or_else(|| {
            JsNativeError::typ()
                .with_message("URLSearchParams.prototype.get called on non-URLSearchParams object")
        })?;
...
fn get_all(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    let params = this_obj
        ...
        .ok_or_else(|| {
            JsNativeError::typ().with_message(
                "URLSearchParams.prototype.getAll called on non-URLSearchParams object",
            )
        })?;
...
fn has_param(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    let params = this_obj
        ...
        .ok_or_else(|| {
            JsNativeError::typ()
                .with_message("URLSearchParams.prototype.has called on non-URLSearchParams object")
        })?;
...
fn set_param(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    let mut params = this_obj
        ...
        .ok_or_else(|| {
            JsNativeError::typ()
                .with_message("URLSearchParams.prototype.set called on non-URLSearchParams object")
        })?;
...
fn sort_params(this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
    ...
    let mut params = this_obj
        ...
        .ok_or_else(|| {
            JsNativeError::typ()
                .with_message("URLSearchParams.prototype.sort called on non-URLSearchParams object")
        })?;
...
fn params_to_string(
    ...
    let params = this_obj
        ...
        .ok_or_else(|| {
            JsNativeError::typ().with_message(
                "URLSearchParams.prototype.toString called on non-URLSearchParams object",
            )
        })?;
...
fn entries(this: &JsValue, _args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    // Clone params before any context calls to avoid RefCell double-borrow
    let params_clone = {
        let params = this_obj
            .downcast_ref::<UrlSearchParamsData>()
            .ok_or_else(|| {
                JsNativeError::typ().with_message(
                    "URLSearchParams.prototype.entries called on non-URLSearchParams object",
                )
            })?;
        params.params.clone()
    };

    let entries: Vec<JsValue> = params_clone
        .iter()
        .map(|(k, v)| {
...
fn keys(this: &JsValue, _args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    let keys_data = {
        let params = this_obj
            ...
            .ok_or_else(|| {
                JsNativeError::typ().with_message(
                    "URLSearchParams.prototype.keys called on non-URLSearchParams object",
                )
            })?;
        params
            .params
    ...
    let array = boa_engine::object::builtins::JsArray::from_iter(keys_data, context);
...
fn values(this: &JsValue, _args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    let values_data = {
        let params = this_obj
            ...
            .ok_or_else(|| {
                JsNativeError::typ().with_message(
                    "URLSearchParams.prototype.values called on non-URLSearchParams object",
                )
            })?;
        params
            .params
    ...
    let array = boa_engine::object::builtins::JsArray::from_iter(values_data, context);
...
fn for_each(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    // Clone params before callback execution to avoid RefCell double-borrow
    let params_clone = {
        let params = this_obj
            .downcast_ref::<UrlSearchParamsData>()
            .ok_or_else(|| {
                JsNativeError::typ().with_message(
                    "URLSearchParams.prototype.forEach called on non-URLSearchParams object",
                )
            })?;
        params.params.clone()
    };
...
mod tests {
    ...
    fn test_url_constructor() {
        ...
        assert_eq!(
            result,
            JsValue::from(js_string!("https://example.com/path?query=1#hash"))
        );
    }
    ...
    fn test_url_search_params_exists() {
        let mut context = create_test_context();
        let result = context
            .eval(Source::from_bytes("typeof URLSearchParams"))
            .unwrap();
        assert_eq!(result, JsValue::from(js_string!("function")));
    }
